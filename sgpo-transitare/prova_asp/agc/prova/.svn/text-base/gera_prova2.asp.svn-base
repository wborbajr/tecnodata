...
<%
     ...
     Dim TblAcessos, TblGeraProva, v1, v2, TblProvaGerada, dia, mes, ano
     ...
     set TblGeraProva = ObjConn.Execute ("SELECT questoes.codQuestao AS codQuestao, questoes.correta AS questao_correta, questoes.codDisciplina AS codDisciplina, questoes.codDificuldade AS codDificuldade FROM questoes INNER JOIN parametrizacao ON questoes.codDisciplina = parametrizacao.codDisciplina INNER JOIN dificuldade ON questoes.codDificuldade = dificuldade.codDificuldade WHERE parametrizacao.codDisciplina AND parametrizacao.codDificuldade AND parametrizacao.codProva = '"& Request("codProva") &"' ORDER BY RAND() LIMIT "&TblConfigProva("questoes")&"")
			...
			<div align="center">
				<table border="0" cellpadding="0" cellspacing="1" width="100%" id="table2" bgcolor="#C0C0C0">
					...
					<% while not tblGeraProva.EOF %>
						...
						objconn.Execute ("INSERT INTO provas_geradas(codCliente,codCandidato,codProva,codDisciplina,codDificuldade,codQuestao,questao_correta,dia,mes,ano) VALUES ('"& Session("codCliente") &"','"& Session("codCandidato") &"','"& Request("codProva") &"','"& TblGeraProva("codDisciplina") &"','"& TblGeraProva("codDificuldade") &"','"& TblGeraProva("codQuestao") &"','"& TblGeraProva("questao_correta") &"','"& dia &"','"& mes &"','"& ano &"')")
						%>
					<tr>
