...
//	if (($_SERVER["REQUEST_METHOD"] == "POST") || (!empty($_POST["txtNome"])){
	if ($_SERVER["REQUEST_METHOD"] == "POST"){
		...
		if(!empty($_POST["txtNome"])){
			$SQL = "SELECT e.idesc, e.nome_fantasia, e.cidade, e.uf, p.nome
					 ...
					 AND e.nome_fantasia LIKE '%".$_POST["txtNome"]."%' 
...
function leva_dados(id_esc,nome) {
	with (opener.document.frmAgenda) {
		txtCodEscola.value=id_esc;
		txtEscola.value=nome;
		aux.value=id_esc;
    }
}
		      ...
		      <table width="99%"  border="0" cellspacing="2" cellpadding="2">
		          ...
		          ?>
                        <tr bgcolor="#EFEFEF">
                          ...
                          <td align="center">
						  <a onClick="leva_dados(<? echo $dados["idesc"]; ?>,'<? echo $dados["nome_fantasia"]; ?>'); window.close();" style="cursor:pointer;"><img src="images/ok.gif" alt="..." ></a>
						  </td>
                          </tr>
					<?
					}
